package RS232_DEFINITIONS is
   ...
   constant CLK_F                   : integer := 50000000;                   -- clock frequency of crystal (50 MHz)
   constant BIT_INTERVAL_TIME       : integer := (CLK_F/BAUD_RATE)-1;         -- length of 1 bit interval
   constant FIRST_BIT_INTERVAL_TIME : integer := (3*CLK_F)/(2*BAUD_RATE);     -- when RxD goes '0', wait for 1.5 of bit interval to reach the middle of the 1st data bit
...
architecture RS232_BODY of RS232 is
   ...
   -- RXD ----------------------------------------------
   -----------------------------------------------------
   type   TYPE_RX_STATE is (START, INIT, RECEIVE, ACK);

   signal RX_BIT_INTERVAL            : unsigned(15 downto 0);
   ...
   signal TX_BIT_INTERVAL         : unsigned(15 downto 0);
...
begin
   ...
   begin
      if CLK = '1' and CLK'event then
         if RX_LOAD_BIT_INTERVAL = '1' then
            RX_BIT_INTERVAL <= to_unsigned(BIT_INTERVAL_TIME,16);
         elsif RX_LOAD_FIRST_BIT_INTERVAL = '1' then
            RX_BIT_INTERVAL <= to_unsigned(FIRST_BIT_INTERVAL_TIME,16);
         elsif RX_EN_BIT_INTERVAL = '1' then
            RX_BIT_INTERVAL <= RX_BIT_INTERVAL - 1;
         end if;
      end if;
   end process RX_BIT_INTERVAL_MEASUREMENT;
   
   RX_BIT_INTERVAL_END <= '1' when (RX_BIT_INTERVAL = 0) else
                          '0';


   ---------------------------------------------
   -- BIT COUNTING -----------------------------
   -- counts bits to be received
   ---------------------------------------------
   RX_BIT_COUNTING : process (CLK)
   begin
      if CLK = '1' and CLK'event then
         if RX_LOAD_BIT_COUNTER = '1' then
            RX_BIT_COUNTER <= to_unsigned(NO_OF_TRANSFERRED_BITS,4);
         elsif RX_EN_BIT_COUNTER = '1' then
            RX_BIT_COUNTER <= RX_BIT_COUNTER - 1;
         end if;
      end if;
   end process RX_BIT_COUNTING;
   
   RX_BIT_COUNTER_END <= '1' when (RX_BIT_COUNTER = 0) else
                         '0';
   ...
   begin
      ...
      case RX_STATE is
         ...
         when INIT    =>    RX_NEXT_STATE <= RECEIVE;
                            ------------------------
                            RX_LOAD_FIRST_BIT_INTERVAL <= '1';

         when RECEIVE => if RX_BIT_INTERVAL_END = '0' then
   ...
   end process RX_FSM_TRANSITION_AND_OUTPUT;
   ...
   begin
      if CLK = '1' and CLK'event then
         if TX_LOAD_BIT_INTERVAL = '1' then
            TX_BIT_INTERVAL <= to_unsigned(BIT_INTERVAL_TIME,16);
         elsif TX_EN_BIT_INTERVAL = '1' then
            TX_BIT_INTERVAL <= TX_BIT_INTERVAL - 1;
         end if;
      end if;
   end process TX_BIT_INTERVAL_MEASUREMENT;
   
   TX_BIT_INTERVAL_END <= '1' when (TX_BIT_INTERVAL = to_unsigned(0,16)) else
                          '0';


   ---------------------------------------------
   -- BIT COUNTING -----------------------------
   -- counts bits to be sent
   ---------------------------------------------
   TX_BIT_COUNTING : process (CLK)
   begin
      if CLK = '1' and CLK'event then
         if TX_LOAD_BIT_COUNTER = '1' then
            TX_BIT_COUNTER <= to_unsigned(NO_OF_TRANSFERRED_BITS+2,4);
         elsif TX_EN_BIT_COUNTER = '1' then
            TX_BIT_COUNTER <= TX_BIT_COUNTER - 1;
         end if;
      end if;
   end process TX_BIT_COUNTING;
   
   TX_BIT_COUNTER_END <= '1' when (TX_BIT_COUNTER = 0) else
                         '0';
   ...
   end process TX_FSM_TRANSITION_AND_OUTPUT;
   ...
   end process TX_SHIFT_REG;
   
   
   TXD <= TXD_DATA_REG(0);


end architecture RS232_BODY;